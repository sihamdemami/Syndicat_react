...
const EditProfile = () => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (response.status === 200 && response.data) {
          const { firstname, lastname, birthday, genre, role, email, phonenumber, CIN, image } = response.data;
          setFormData({ firstname, lastname, birthday, genre, role, email, phonenumber, CIN });
          setImage(image); 
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (image) {
      updatedFormData.append('image', image); // Ajoute l'image existante à l'objet FormData
    }
  ...
  // Utilisation de "image" dans le composant
  console.log(image); // Affiche le nom du fichier dans la console
